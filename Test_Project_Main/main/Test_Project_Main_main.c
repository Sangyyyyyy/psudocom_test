...
#define SOFTAP_ESP_SSID   "IM_SSID"
...
vPeriod_change_task (void *pvParam)
{
  ...
  //httpd_resp_sendstr_chunk (req,
  //                          "else {alert(\"failed to send.\");}} catch (error) {console.error(\"err:\", error);alert(\"error occured while sending data to server.\");}} else {alert(\"1~10000.\");}});");

/*        if (strlen (SOFTAP_ESP_SSID) == 0)
        {
